  ...
  private async initializeIndices() {
    try {
      ...
      const searchLogsCreated = await this.elasticsearchService.createIndex(
        DEFAULT_ES_INDICES.SEARCH_LOGS,
        DEFAULT_INDEX_SETTINGS,
        SEARCH_LOGS_MAPPING,
      );

      // Create user events index
      const userEventsCreated = await this.elasticsearchService.createIndex(
        DEFAULT_ES_INDICES.USER_EVENTS,
        DEFAULT_INDEX_SETTINGS,
        USER_EVENTS_MAPPING,
      );

      if (programsCreated && universitiesCreated && searchLogsCreated && userEventsCreated) {
