...
export class ProgramsEsInitService implements OnModuleInit {
  ...
  private async initializeIndices() {
    try {
      ...
      const programsSuccess = await this.mappingRegistryService.applyMapping(ES_INDICES.PROGRAMS);
      
      if (programsSuccess) {
        this.logger.log('Programs Elasticsearch index initialized successfully');
      } else {
