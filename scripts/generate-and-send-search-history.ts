...
const SearchResourceEnumValues = [
  SearchResourceEnum.PROGRAM,
  SearchResourceEnum.UNIVERSITY,
  SearchResourceEnum.CAMPUS,
] as const;
...
async function main() {
  ...
  const count = 10;
  const fakeSearchHistories = Array.from(
    { length: count },
    generateFakeSearchHistory,
  );
